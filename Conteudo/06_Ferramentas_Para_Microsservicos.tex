...
% TODO: Elaborar sobre FaaS e serverless. >> FaaS é um subconjunto da computação serverless
...
Ao usar esses serviços, uma grande parte do trabalho de configuração e gerenciamento da infraestrutura é terceirizada para esses provedores, que quase certamente são melhores em lidar com essas questões. No caso de serviços de computação \emph{serverless}, \textbf{todo} esse trabalho é abstraido, assim sendo de interesse especial às aplicações com arquitetura de microsserviços, por precisarem de muita infraestrutura. Outra vantagem é a disponibilidade global dos centros de dados dos provedores de nuvem, que permite que componentes da aplicação sejam distribuídos geograficamente, possivelmente reduzindo a latência e melhorando a experiência do usuário final.

% (ou todo o, no caso de soluções \emph{serverless})
% essa última que é de , por abstratir totalmente preocupações de infraestrutura.
% podendo oferencer desde máquinas virtuais e contêineres até bancos de dados e soluções de FaaS

% Na computação serverless, toda configuração e gerenciamento	da infraestrutura ou plataforma é abstraido.
% FaaS é um subconjunto da computação serverless. Trata-se de executar código (funções) em resposta a eventos, sem precisar nem se pensar em infraestrutura.

% Ao usar serviços de computação \emph{serverless}, que abstrai totalmente a configuração de infraestrutura.
...
% Function as a Service (FaaS) is a cloud computing model that enables developers to execute individual functions in response to specific events without the need to manage the underlying infrastructure. Each function operates as an independent, self-contained unit of execution that is triggered by events such as HTTP requests, database changes, or message queue inputs. https://www.ibm.com/think/topics/faas?utm_source=chatgpt.com
...
% Entretanto, é importante ter em mente o \hyperref[teorema-cap]{teorema CAP} ao escalar servidores de bancos de dados horizontalmente, pois dependendo do SGBD escolhido, haverá ou consistência estrita de dados ou disponibilidade contínua de todos os nós no \emph{cluster} distribuído. \cite{mongodb-sql-nosql}
