...
Requisições HTTP para uma API é o método mais usado para realizar comunicação síncrona entre microsserviços. \cite{MartinFowlerMicroservices}. Uma requisição HTTP é feita por um cliente, para um dado \emph{host} em um servidor, com o propósito de acessar um recurso nesse servidor. Por usar o \emph{Transmition Control Protocol} (TCP), é um método de comunicação confiável, mas não tão eficiente quanto poderia ser.
