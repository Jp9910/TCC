...
A observabilidade se trata de permitir a observação do estado de um sistema por meio da externalização de seu comportamento. O monitoramento engloba a observabilidade e se trata de acompanhar o estado de um sistema por meio de registros e ações que podem ser tomadas como resposta a eles. O monitoramento é fundamental para promover o funcionamento adequado de qualquer aplicação, especialmente em arquiteturas distribuidas, como a de microsserviços. Nelas, esse se torna ainda mais importante e complexo. Além disso, a automação e o monitoramento estão altamente ligados.
...
\subsection{Registros}\label{subsecao-registros}

Um registro, ou \emph{log}, descreve o que aconteceu em um dado momento em um dado processo, provendo informações sobre o estado e a saúde dele. Manter um histórico de registros do microsserviço é uma forma simples e eficiente de se implementar monitoramento, e é fortemente indicado. Para manter registros, pode-se desenvolver um serviço dedicado a isso ou utilizar bibliotecas, para poder ser reutilizado onde necessário.

Para facilitar uma eficiente escrita, leitura e operação dos registros, deve-se padronizar o formato dos registros em todos os microsserviços e diferenciar entradas de erros, de avisos e de informação. Além disso, eles devem ser agregados e organizados em um único lugar para que possam ser facilmente consultados.
